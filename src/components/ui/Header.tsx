...
export default function Header({ title, description, icon: Icon }: HeaderProps) {
    return (
        <header className="flex items-center gap-3 sm:gap-6 mb-4">
            <div className="p-2 sm:p-3 bg-white/5 rounded-xl">
                {Icon ? <Icon color="#63a4fa" /> : <Braces color="#63a4fa" />}
            </div>
            <div>
                <h1 className="text-lg sm:text-lg font-bold mb-1 sm:mb-2">{title}</h1>
                {description && <p className="text-sm sm:text-sm text-gray-400">{description}</p>}
